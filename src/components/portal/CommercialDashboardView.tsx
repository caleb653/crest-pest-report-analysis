                ...
                {upcoming.map(s => {
                  ...
                  <Card key={s.id}>
                    <CardContent className="p-3 space-y-2">
                      ...
                      <div className="rounded-md border border-amber-500/30 bg-amber-50/60 dark:bg-amber-500/5 overflow-hidden">
                        <button
                          type="button"
                          onClick={() => setExpandedConditions(p => ({ ...p, [s.id]: !p[s.id] }))}
                          className="w-full flex items-center justify-between gap-2 text-left px-3 py-2.5 hover:bg-amber-500/10 transition-colors"
                        >
                          <span className="inline-flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide text-amber-700 dark:text-amber-400">
                            <AlertTriangle className="w-3.5 h-3.5" /> Active Conditions
                          </span>
                          <ChevronDown className={`w-4 h-4 text-amber-700/70 dark:text-amber-400/70 transition-transform ${expandedConditions[s.id] ? "rotate-180" : ""}`} />
                        </button>
                        {expandedConditions[s.id] && (
                          <div className="px-2 pb-2 pt-1 border-t border-amber-500/20">
                            <ConditionsReportSection
                              services={[s as any]}
                              onSaveServiceReportData={persistServiceReportData}
                              includeUndated
